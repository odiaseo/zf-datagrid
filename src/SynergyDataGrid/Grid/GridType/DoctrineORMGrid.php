	...
	public function setGridIdentity( $entityClassName, $gridId = '', $idSuffix = null, $displayTree = true ) {
		...
		if ( $displayTree ) {
			...
			if ( 'Gedmo\Tree\Entity\Repository\NestedTreeRepository' == $mapping->customRepositoryClassName ) {
				...
				$config ['grid_options']   = $utils->arrayMergeRecursiveCustom( $config ['grid_options'],
					$treeOptions );
			}
		}
		...
		if ( ! empty( $config ['grid_options']['onSelectRow'] )
		     && is_string( $config ['grid_options']['onSelectRow'] )
		) {
